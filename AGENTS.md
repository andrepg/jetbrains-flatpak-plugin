...
- **Flatpak runs**: `src/main/kotlin/io/github/andrepg/flatpak/runs/`
  ...
  - `cleanup/`: blocking pre-step implementations — `DeepCleanExecutor.kt` deletes deep-clean targets through the IntelliJ VFS inside a `WriteCommandAction` (pooled-thread caller required); `StaleFuseMountCleaner.kt` (pure JDK) unmounts stale rofiles-fuse mounts under the build dir, fail-open
